...
%if 0%{?suse_version}
BuildRequires: go >= 1.21 ImageMagick >= 6, desktop-file-utils >= 0.23, git >= 2.27.0, appstream-glib >= 0.7.16, npm >= 8.11.0{{ range $pkg := .ExtraPackages }}, {{ $pkg.Name }} >= {{ $pkg.Version }}{{ end }}
%else
BuildRequires: golang >= 1.21 ImageMagick >= 6, desktop-file-utils >= 0.23, git >= 2.27.0, libappstream-glib >= 0.7.14, npm >= 8.11.0{{ range $pkg := .ExtraPackages }}, {{ $pkg.Name }} >= {{ $pkg.Version }}{{ end }}
%endif
