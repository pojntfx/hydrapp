...
apt install -y com.pojtinger.felix.hydrapp.example

# Alternatively install from source
apt install -y dpkg-dev

apt -y build-dep com.pojtinger.felix.hydrapp.example
apt source -y --build com.pojtinger.felix.hydrapp.example
apt install -y ./com.pojtinger.felix.hydrapp.example_*.deb
```
...
# Install binary package
if [ "${ID}" = "opensuse-tumbleweed" ]; then
    zypper install -y com.pojtinger.felix.hydrapp.example
else
    dnf install -y com.pojtinger.felix.hydrapp.example
fi
...
if [ "${ID}" = "opensuse-tumbleweed" ]; then
    ...
    zypper -n source-install com.pojtinger.felix.hydrapp.example

    rpmbuild -ba /usr/src/packages/SPECS/com.pojtinger.felix.hydrapp.example.spec

    zypper --no-gpg-checks install -y /usr/src/packages/RPMS/"$(uname -m)"/com.pojtinger.felix.hydrapp.example-*.rpm
else
    dnf install -y rpm-build

    dnf download --source -y com.pojtinger.felix.hydrapp.example
    dnf builddep -y com.pojtinger.felix.hydrapp.example-*.rpm
    rpmbuild --rebuild com.pojtinger.felix.hydrapp.example-*.rpm
    dnf install -y ~/rpmbuild/RPMS/"$(uname -m)"/com.pojtinger.felix.hydrapp.example-*.rpm
fi
```
...
flatpak install -y com.pojtinger.felix.hydrapp.example
```
